// Desktop.C	
// updated	2015/06/26 by Dave Henderson (dhenderson@cliquesoft.org or support@cliquesoft.org)

#include "config.h"

#if MULTIPLE_DESKTOPS				// if the WM is configured to use desktops, then add the below functionality!
...
#include <stdlib.h>
	...
	int Desktop::available_number() {
		...
		for (Desktop* d = first; d;) {
			if (d->number_ == n) {n++; d = first;} else { d = d->next; }
		}
		return n;
	}
	...
	static void send_desktops() {
		...
		XChangeProperty(fl_display, fl_xid(Root), _win_workspace_names, XA_STRING, 8, PropModeReplace, (unsigned char *)buffer, p-buffer-1);
	}
	...
	void init_desktops() {
		dont_send = 1;
		int length;
		char* buffer = (char*)getProperty(fl_xid(Root), _win_workspace_names, XA_STRING, &length);
		if (buffer) {
			char* c = buffer;
